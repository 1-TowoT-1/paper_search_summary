...
def show_response(response: requests.Response) -> Any | None:
    try:
        body = response.json()
    except ValueError:
        body = response.text
...
def request(method: str, path: str, **kwargs: Any) -> requests.Response:
    ...
    return requests.request(method, api_url(path), headers=headers, timeout=timeout, **kwargs)
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        if st.button("检查后端健康状态", use_container_width=True):
            try:
                show_response(requests.get(api_url("/health"), timeout=10))
        ...
        if st.session_state.access_token:
            ...
            with login_tab:
                ...
                if st.button("登录", use_container_width=True):
                    ...
                    try:
                        response = requests.post(api_url("/api/auth/login"), json=payload, timeout=30)
                        ...
                        if isinstance(body, dict) and body.get("access_token"):
                            ...
                            st.session_state.token_type = body.get("token_type", "bearer")
            ...
            with register_tab:
                ...
                if st.button("注册", use_container_width=True):
                    ...
                    try:
                        show_response(requests.post(api_url("/api/auth/register"), json=payload, timeout=30))
                    except requests.RequestException as exc:
...
def render_search() -> None:
    ...
    with st.expander("查询改写调试"):
        ...
        if st.button("生成改写"):
            try:
                show_response(requests.post(api_url("/api/search/rewrite"), params={"q": rewrite_query}, timeout=30))
            except requests.RequestException as exc:
...
def render_import() -> None:
    ...
    limit = st.slider("导入数量", 1, 100, 20, key="import_limit")
    include_pdf = st.checkbox("尝试解析 PDF 全文")

    if st.button("创建导入任务"):
        payload = {"query": query, "sources": sources, "limit": limit, "include_pdf": include_pdf}
        try:
            show_response(request("POST", "/api/papers/import", json=payload, timeout=180))
        except requests.RequestException as exc:
            st.error(f"导入请求失败: {exc}")
...
def render_papers() -> None:
    st.subheader("文献详情与总结")
    paper_id = st.text_input("文献 ID", key="paper_detail_id")
    col1, col2 = st.columns(2)
